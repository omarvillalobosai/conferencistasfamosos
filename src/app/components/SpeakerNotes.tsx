...
export default function SpeakerNotes({ speakerId, notes, contacts, onChange, onUseFee, onUsePhone }: {
  ...
  const found = source === 'whatsapp' ? detectWhatsApp(body) : { fees: [], phones: [] };
  const save = async () => {
    ...
    try {
      ...
      onChange([data as SpeakerNote, ...notes]); setBody(''); setMessage('Nota guardada.');
  ...
  return <section className="cf-section"><h2 className="cf-h2">Pegar de WhatsApp</h2>
    <div className="cf-form cf-card">
      ...
      {(found.fees.length > 0 || found.phones.length > 0) && <p className="cf-note">Encontré estos datos. Elige uno y luego guarda la ficha.</p>}
